...
////////***Beginning Libraries***////////
      ...
      int lighCheckTime = 500; //how often we check light sensor. 10 minutes will be 600000 milliseconds
...
//    RTC.setAlarm(ALM1_MATCH_SECONDS, 0, 2, 0, 1);    //daydate parameter should be between 1 and 7
//    RTC.alarm(ALARM_1);                   //ensure RTC interrupt flag is cleared
//    RTC.alarmInterrupt(ALARM_1, true);

    //Set an alarm every 10 minutes.
    RTC.setAlarm(ALM1_EVERY_MINUTE, 0, 0, 2, 1);    //daydate parameter should be between 1 and 7
    RTC.alarm(ALARM_1);                   //ensure RTC interrupt flag is cleared
    RTC.alarmInterrupt(ALARM_1, true);
    Serial.println ("Alarm 1 is setup");
    

    //Set a daily alarm.
    RTC.setAlarm(ALM2_MATCH_HOURS, 0, 52, 0, 1);    //daydate parameter should be between 1 and 7
    RTC.alarm(ALARM_2);                   //ensure RTC interrupt flag is cleared
    RTC.alarmInterrupt(ALARM_2, true);
    Serial.println ("Alarm 2 is setup");

//
//    RTC.setAlarm(ALM1_MATCH_HOURS, 0, 30, 23, 1);    //daydate parameter should be between 1 and 7
//    RTC.alarm(ALARM_1);                   //ensure RTC interrupt flag is cleared
//    RTC.alarmInterrupt(ALARM_1, true);
//    Serial.println ("Alarm 1 is on");
//
//    RTC.setAlarm(ALM2_EVERY_MINUTE, 0, 2, 0, 1);    //daydate parameter should be between 1 and 7
//    RTC.alarm(ALARM_2);                   //ensure RTC interrupt flag is cleared
//    RTC.alarmInterrupt(ALARM_2, true);
//    Serial.println ("Alarm 2 is on too?");
